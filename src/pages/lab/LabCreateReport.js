...
import {clearReports} from "../../redux/actions/reports";

class LabCreateReport extends React.Component {
    ...
    saveAction = async (isReloadAction = true) => {
        ...
        formData.append("lab", this.state.user.institution.id);
    ...
    renderPage = () => {
        if (this.state.isDataLoad) {
            if (this.state.user.institution === undefined) {
                return <Redirect to="/"/>
            }
    ...
    render() {
        if (this.state.reloadPage) {
            ...
            return <Redirect to="/lab/create-report/"/>;
        }
...
export default connect(null, {clearReports})(LabCreateReport);