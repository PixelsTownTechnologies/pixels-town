...
class CreateHospital extends React.Component {
    ...
    FieldWebsite = (formProps) => {
        return <TextField autoComplete={true} required={false} placeholder="http://..."
                          label="Hospital website" {...formProps.input}/>;
    };
    ...
    submitActionCore = async (formValues) => {
        ...
        const url = "user/" + this.props.user.id + "/create_hospital/";
            ...
            <Form onSubmit={this.props.handleSubmit(this.submitActionCore)}>
                <Grid>
                    ...
                    <Row>
                        <Column size={16}>
                            <LabelTeal style={{fontSize: "2.0em"}}
                                       text={<div><i className="hospital icon"/>Create Hospital</div>}/>
                        </Column>
                    </Row>
                    <Row/>
                    <Row/>
                    ...
                    <Row>
                        <Column size={1}/>
                        <Column size={7}>
                            <div style={{textAlign: "center", marginTop: "-40px", fontSize: "1.2em"}}>
                                <br/>
                                You work in hospital ?
                                <br/>
                                <div style={{width: '70%', marginLeft: '15%'}}>
                                    <Link to="/join-hospital">Join hospital</Link>
                                </div>
                            </div>
                        </Column>
                        <Column size={1}/>
    ...
    render() {
        if (this.props.user.institution !== undefined || this.props.user.join_institution !== undefined) {
            return <Redirect to="/"/>;
        }
        return (
            <UserType
...
export default reduxForm({
    form: 'createHospital'
})(connect(mapStateToProps, {addHospital, getUser})(CreateHospital));