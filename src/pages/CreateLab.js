...
class CreateLab extends React.Component {
    ...
    FieldWebsite = (formProps) => {
        return <TextField autoComplete={true} required={false} placeholder="http://..."
                          label="Lab website" {...formProps.input}/>;
    };
    ...
    submitActionCore = async (formValues) => {
        ...
        const url = "user/" + this.props.user.id + "/create_lab/";
                    ...
                    <Row>
                        <Column size={16}>
                            <LabelTeal style={{fontSize: "2.0em"}}
                                       text={<div><i className="dna icon"/>Create Lab</div>}/>
                        </Column>
                    </Row>
                    <Row/>
                    <Row/>
...
export default reduxForm({
    form: 'createLab'
})(connect(mapStateToProps, {addHospital, getUser})(CreateLab));