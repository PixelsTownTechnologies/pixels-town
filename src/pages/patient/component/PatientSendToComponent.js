...
class PatientSendToComponent extends React.Component {
    ...
    componentDidMount = async () => {
        const url = "/user/" + this.props.user.id + "/get_send_users_patients/?search=" + this.state.filterValue;
    ...
    selectAction = (item) => {
        ...
        this.setState({selectedItems: [...this.state.selectedItems, item], items: items});
        if (this.props.pushSelected !== undefined) {
            this.props.pushSelected([...this.state.selectedItems, item]);
        }

    };

    unselectAction = (item) => {
        const items = this.state.selectedItems.filter(function (value, index, arr) {
            return value !== item;
        });
        this.setState({items: [...this.state.items, item], selectedItems: items});
    };
    ...
    render() {
        ...
        return (
            <div className="ui fluid search selection dropdown active visible" onClick={this.setDisplay}>
                ...
                />
                <div className="default text">Send to:</div>
...
export default PatientSendToComponent;